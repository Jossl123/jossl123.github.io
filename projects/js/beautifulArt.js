...
let circleRadius
let center
let rotateAngle = Math.PI * 2 / pointsNb
...
function setup() {
    ...
    for (let i = 0; i <= pointsNb; i++) {
        for (let j = 0; j < 10; j++) {
            let angle = rotateAngle * i
            let vector = createVector(circleRadius, 0).rotate(angle)
            let finalPos = createVector(center.x, center.y).add(vector)
